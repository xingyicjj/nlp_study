#------------微调模型training_bert.py------------#
#创建一个名为waimai_bert的虚拟环境
#创建/waimai_bert/dataset，存放数据集作业数据-waimai_10k.csv
#创建/waimai_bert/model，存放bert-base-chinise模型
#创建/waimai_bert/weights，作为训练模型后的权重文件存储路径
#创建/waimai_bert/self_code/code/training_bert.py，输入以下代码
...
# 加载和预处理数据
dataset_df = pd.read_csv('../dataset/作业数据-waimai_10k.csv', sep=',', header=0)
# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换前500个标签，得到数字标签
labels = lbl.fit_transform(dataset_df['label'].values[:100])
# 提取前500个文本内容
...
model = BertForSequenceClassification.from_pretrained(
    'bert-base-chinese', num_labels=12 # 判别式模型，分多少类
)
