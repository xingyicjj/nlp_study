...
# 加载和预处理数据
dataset_df = pd.read_csv("./waimai_10k.csv", sep=",")



# 分割数据为训练集和测试集
x_train, x_test, train_labels, test_labels = train_test_split(
    list(dataset_df["review"].values),             # 文本数据
    dataset_df["label"].values,            # 对应的数字标签
    test_size=0.2,     # 测试集比例为20%
    stratify=dataset_df["label"].values    # 确保训练集和测试集的标签分布一致
)
