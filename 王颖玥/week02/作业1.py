...
"""
调整了模型的层数和节点数，采用3层，隐藏层每层128节点来拟合
并绘制出了损失值趋势图，具体见「说明文档」
"""
...
class CharBoWDataset(Dataset):
    def __init__(self, texts, labels, char_to_index, max_len, vocab_size):
        ...
        self.labels = torch.tensor(labels, dtype=torch.long)   # 标签张量
        self.char_to_index = char_to_index   # 字符→索引映射
        self.max_len = max_len   # 文本最大长度
        self.vocab_size = vocab_size   # 词汇量
        self.bow_vectors = self._create_bow_vectors()  # 预生成所有文本的词袋向量
    ...
    def __len__(self):   # 必须实现：返回数据集样本总数
        return len(self.texts)

    def __getitem__(self, idx):  # 必须实现：根据索引返回单个样本（特征+标签）
...
network_info = "2 Level, 128 Neurons"
# network_info = "3 Level, 128, 128 Neurons"
plt.figure(figsize=(13, 6))
plt.plot(range(1, num_epochs + 1), epoch_losses, marker='o', color='b', linewidth=2)
plt.xlabel('Epoch')
