...
# 加载和预处理数据
df = pd.read_csv('./作业数据-waimai_10k.csv', sep=',', header=None)

# 随机抽取500个样本
dataset_df = df.sample(n=500, random_state=42)  # 设置random_state以确保结果可复现

# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换500个标签，得到数字标签
...
model = BertForSequenceClassification.from_pretrained(
    './models/bert-base-chinese', num_labels=len(lbl.classes_)  # 判别式模型，分多少类
)
